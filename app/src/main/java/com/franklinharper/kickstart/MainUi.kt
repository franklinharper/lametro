...
class MainUiImpl(
    ...
    override fun onCreate(mainPresenter: MainPresenter) {
        ...
        configureRecyclerView(mainPresenter)
        setLocationButton.setOnClickListener { mainPresenter.onSetLocationClick() }
    }

    private fun configureRecyclerView(mainPresenter: MainPresenter) {
        searchResultsList.layoutManager = LinearLayoutManager(
            activity,
            RecyclerView.VERTICAL,
            false
        )
        searchResultsList.addItemDecoration(
            DividerItemDecoration(ContextCompat.getDrawable(activity, R.drawable.divider))
        )
        searchAdapter.setRetryClickLister { mainPresenter.onRetryClick() }
